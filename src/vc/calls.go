...
func (c *TelegramCalls) getClientIndex(chatID int64) (int, error) {
	...
	if err != nil {
		slog.Info(
			"[TelegramCalls] DB.GetAssistant error",
			"error",
			err,
		)
		assignedIndex = -1
	...
	n, err := rand.Int(
		rand.Reader,
		big.NewInt(int64(totalClients)),
	)
	if err != nil {
		slog.Info(
			"[TelegramCalls] Could not generate a random number",
			"error",
			err,
		)
		...
		if assignedIndex == -1 && chatID != 0 {
			if _, err := db.Instance.AssignAssistant(
				chatID,
				newClientIndex,
			); err != nil {
				logger.Info(
					"[TelegramCalls] DB.AssignAssistant error",
					"error",
					err,
				)
			}
...
func (c *TelegramCalls) GetGroupAssistant(
	chatID int64,
) (*Assistant, int, error) {
	...
	if !ok {
		return nil, -1, fmt.Errorf(
			"no ntgcalls instance was found for client index %d",
			clientIndex,
		)
	}
...
// playSong downloads and plays a single song. It sends a message to the chat
// to indicate the download status and updates it once playback begins.
func (c *TelegramCalls) playSong(
	bot *td.Client,
	chatID int64,
	song *utils.CachedTrack,
) error {
	reply, err := bot.SendTextMessage(
		chatID,
		fmt.Sprintf("Downloading %s...", song.Name),
		nil,
	)
	if err != nil {
		slog.Info(
			"[playSong] Failed to send message",
			"error",
			err,
		)
		return err
	...
	isAutoplay := strings.EqualFold(
		strings.TrimSpace(song.User),
		"Autoplay",
	)

	if isAutoplay && strings.TrimSpace(song.FilePath) == "" {
		var downloadErr error

		// Initial download attempt + one retry.
		for attempt := 1; attempt <= 2; attempt++ {
			song.FilePath, downloadErr = dl.DownloadCachedTrack(
				song,
				bot,
			)

			if downloadErr == nil &&
				strings.TrimSpace(song.FilePath) != "" {
				break
			}
			...
			slog.Warn(
				"[playSong] Autoplay download failed",
				"chat_id",
				chatID,
				"track_id",
				song.TrackID,
		...
		if downloadErr != nil ||
			strings.TrimSpace(song.FilePath) == "" {
			if downloadErr == nil {
				downloadErr = errors.New(
					"download returned an empty file path",
				)
			}

			cache.ChatCache.SetAutoplay(chatID, false)

			stopErr := c.Stop(chatID, false)

			_, _ = reply.EditText(
				bot,
				"⚠️ Autoplay download failed after one retry. Voice chat ended.",
				nil,
			)

			if stopErr != nil {
				return fmt.Errorf(
					"autoplay download failed after one retry: %v; failed to stop voice chat: %w",
					downloadErr,
					stopErr,
				)
			}
	...
	} else if err = c.downloadAndPrepareSong(
		bot,
		song,
		reply,
	); err != nil {
	...
	if err != nil {
		slog.Info(
			"[playSong] Failed to edit message",
			"error",
			err,
		)
		return nil
...
func (c *TelegramCalls) Pause(
	...
	if err != nil {
		slog.Warn(
			"[Pause] Failed to pause the call",
			"error",
			err,
			"index",
			index,
...
// PlayedTime retrieves the elapsed time of the current playback.
func (c *TelegramCalls) PlayedTime(
	chatId int64,
) (uint64, error) {
	call, index, err := c.GetGroupAssistant(chatId)
	if err != nil {
...
func (c *TelegramCalls) SeekStream(
	bot *td.Client,
	chatID int64,
	filePath string,
	toSeek int,
	...
	if toSeek < 0 || duration <= 0 {
		return errors.New(
			"invalid seek position or duration. The position must be positive and the duration must be greater than 0",
		)
	}
...
func (c *TelegramCalls) RegisterHandlers(
	client *td.Client,
) {
	c.startAutoLeave(
		context.Background(),
	...
	for _, call := range c.assistants {
			...
			func(
				...
				if err := c.PlayNext(
					...
					call.App.Logger.Warnf(
						"[OnStreamEnd] Failed to play the song: %v",
						err,
					)
				}
			},
		...
		go func() {
			_, err := call.App.SendMessage(
				client.Me.Usernames.EditableUsername,
				"/start",
			)
			if err != nil {
				call.App.Logger.Warnf(
					"failed to start bot: %v",
					err,
				)
			}
